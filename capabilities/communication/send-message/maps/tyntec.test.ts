...
const recipient = process.env.COMMUNICATION_SENDMESSAGE_TYNTEC_TO
let profile: any;
let provider: any;
...
describe('communication/send-message/tyntec', () => {
  ...
  it('sends a message', async () => {
    ...
    const result = await useCase.perform({ to: recipient, from: 'tyntec', text: 'Hello World!', channel: 'sms' }, { provider });
  ...
  it('retrieves message status', async () => {
    ...
    const sendMessageResult = await sendMessageUseCase.perform({ to: recipient, from: 'tyntec', text: 'Hello World!', channel: 'sms' }, { provider });
