...
describe('communication/send-message/twilio', () => {
  ...
  it('sends a message', async () => {
    ...
    const result = await useCase.perform({ to: recipient, text: 'Hello World!'}, { provider });
  ...
  it('retrieves message status', async () => {
    ...
    const sendMessageResult = await sendMessageUseCase.perform({ to: recipient, text: 'Hello World!'}, { provider });
