...
describe('vcs/single-file-content/bitbucket', () => {
    it('performs correctly', async () => {
        ...
        await expect(useCase.perform({ owner: 'jakuvacek', repo: 'testrepository', path: 'README.md', ref: 'master' },
            { provider })).resolves.toEqual(ok({ content: expect.stringContaining('README'), encoding: 'utf-8', size: expect.any(Number) }))
    })
})
